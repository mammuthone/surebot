...
const spawPromise = util.promisify(spawn)
...
async function runAdbCommands() {
    try {
        ...
        const pythonProcess = spawn("python3", ["scripts/recognize.py", screenshotPath, 'Accedi']);

        pythonProcess.stdout.on("data", (data) => {
            console.log(`Testo riconosciuto: ${data.toString()}`);
        });

        pythonProcess.stderr.on("data", (data) => {
            console.error(`Errore: ${data.toString()}`);
        });
